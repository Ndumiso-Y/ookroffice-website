...
const Navbar = () => {
  ...
  const divisionMenuItems = [
    { name: 'Divisions Overview', path: '/divisions', logo: null },
    { name: 'Koketso Rakhudu Foundation', path: divisionLinks.krf.url, enabled: divisionLinks.krf.enabled, logo: '/images/KRFLogo.png' },
    { name: 'The Village Economy Indaba', path: divisionLinks.tvei.url, enabled: divisionLinks.tvei.enabled, logo: '/images/TVEILogo.png' },
    { name: 'Coffee with Dr. Rakhudu', path: divisionLinks.coffee.url, enabled: divisionLinks.coffee.enabled, logo: '/images/CoffeeKoketsoLogo.png' },
    { name: 'Business School', path: divisionLinks.businessSchool.url, enabled: divisionLinks.businessSchool.enabled, logo: '/images/BusinessSchoolOfLeadershipExcellenceLogo.png' },
    { name: 'Kgosana Koketso Rakhudu Group', path: divisionLinks.group.url, enabled: divisionLinks.group.enabled, logo: '/images/KKRGLogo.png' },
    { name: 'Tsitsing, Kgotla Ya Rakhudu', path: divisionLinks.kgotla.url, enabled: divisionLinks.kgotla.enabled, logo: '/images/KgotlhaLogo.png' },
  ];
        ...
        <div className="py-4 overflow-y-auto" style={{ maxHeight: 'calc(100vh - 120px)' }}>
          ...
          <div className="border-t border-gray-300 mt-2 pt-2">
            <button
              ...
              className="w-full flex items-center justify-between px-4 py-3 hover:text-green-800 hover:bg-gray-200 transition-colors"
            >
              <div className="flex items-center">
                <FaUsers className="w-5 h-5" />
                <span className="ml-3">Divisions</span>
              </div>
              {divisionsOpen ? <FaChevronUp className="w-4 h-4" /> : <FaChevronDown className="w-4 h-4" />}
            </button>

            {divisionsOpen && (
              <div className="bg-gray-50 border-l-4 border-green-700 ml-4">
                {divisionMenuItems.map((item, index) => (
                  item.enabled !== false && (
                    <Link
                      key={index}
                      to={item.path}
                      onClick={() => setIsOpen(false)}
                      className="flex items-center gap-3 px-6 py-2 text-sm hover:text-green-800 hover:bg-gray-200 transition-colors"
                    >
                      {item.logo && (
                        <img
                          src={item.logo}
                      ...
                      <span className={item.logo ? '' : 'ml-2'}>{item.name}</span>
                    </Link>
                  )
                ))}
              </div>
